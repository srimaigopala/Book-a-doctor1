...
export default function HomePage() {
  ...
  const fetchHomeData = async () => {
    try {
      ...
      // 1. Fetch general platform metrics
      const statsRes = await api.get('/api/dashboard/stats');
      if (statsRes.data.success && statsRes.data.stats) {
        setStats(statsRes.data.stats);
      }

      // 2. Fetch doctors directory to load experience featured profiles
      ...
      // 3. If logged in, load real appointments to extract patient states & schedules
      if (user) {
        const aptRes = await api.get('/api/appointments/all');
        if (aptRes.data.success && aptRes.data.appointments) {
          const allApts = aptRes.data.appointments;
          const userId = user.id || user._id;
          
          // Filters for logged in user (Admin sees all, patient sees their own)
          const myApts = isAdmin 
            ? allApts 
            : allApts.filter(apt => apt.user === userId);
          ...
          // Calculate precise real-time statuses
      ...
      // Fail gracefully so components render fallbacks smoothly
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div id="homepage-root" className="bg-slate-50 min-h-screen pb-16">
      ...
      {!user ? (
        ...
        <section className="relative overflow-hidden bg-white border-b border-slate-150 py-16 md:py-24">
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              ...
              <div className="col-span-1 lg:col-span-7 space-y-6 text-center lg:text-left">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-bold rounded-full bg-blue-50 text-blue-700 border border-blue-105">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start pt-2">
                  ...
                  <Link
                    to="/register"
                    className="px-8 py-4 text-center text-sm font-bold text-slate-700 bg-slate-100 hover:bg-slate-250 rounded-full transition-all cursor-pointer"
                  >
              ...
              <div className="col-span-1 lg:col-span-5 relative flex justify-center">
                <div className="relative w-full max-w-sm">
                  ...
                  <div className="relative bg-white border border-slate-150 p-6 rounded-3xl shadow-xl shadow-blue-100/40 space-y-4 overflow-hidden">
                    ...
                    <div className="opacity-35 select-none pointer-events-none space-y-4">
        ...
        /* ======================== LOGGED IN PATIENT HEALTH DASHBOARD ======================== */
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-sky-500 rounded-3xl p-6 md:p-8 flex flex-col lg:flex-row lg:items-center lg:justify-between text-white shadow-lg shadow-blue-200/40 gap-6 relative overflow-hidden mb-10">
              ...
              <div className="space-y-1.5 relative z-10">
                ...
                <h1 className="text-2xl md:text-3.5xl font-extrabold italic tracking-tight font-display mt-2 flex items-center gap-2">
                ...
                {/* Inline wellness advice alert widget */}
              ...
              {/* Dynamic Clinic Statistics calculated on the fly */}
              <div className="flex items-center gap-3 shrink-0 relative z-10 w-full lg:w-auto justify-start sm:justify-end">
                <div className="bg-white/15 backdrop-blur px-5 py-3 rounded-2xl text-center border border-white/25 flex-1 sm:flex-initial min-w-[110px]">
                  ...
                  <p id="stat-total-appointments" className="text-xl md:text-3xl font-black mt-0.5">{userStats.total}</p>
                </div>
                
                <div className="bg-white/15 backdrop-blur px-5 py-3 rounded-2xl text-center border border-white/25 flex-1 sm:flex-initial min-w-[110px]">
                  <p className="text-[9px] uppercase tracking-widest font-black opacity-85 text-emerald-300">Upcoming</p>
                  <p id="stat-upcoming-appointments" className="text-xl md:text-3xl font-black text-emerald-300 mt-0.5">{userStats.upcoming}</p>
                </div>

                <div className="bg-white/15 backdrop-blur px-5 py-3 rounded-2xl text-center border border-white/25 flex-1 sm:flex-initial min-w-[110px]">
                  <p className="text-[9px] uppercase tracking-widest font-black opacity-85 text-rose-300">Cancelled</p>
                  <p id="stat-cancelled-appointments" className="text-xl md:text-3xl font-black text-rose-305 mt-0.5 text-rose-300">{userStats.cancelled}</p>
                </div>
              </div>
              
              {/* Decorative faint background stethoscope */}
            ...
            {/* Quick Action Navigation Buttons Grid for high usability */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              
              <Link 
                to="/doctors" 
                className="bg-slate-50/80 hover:bg-blue-600 hover:text-white border border-slate-100 hover:border-blue-600 p-6 rounded-3xl shadow-3xs flex justify-between items-center group transition-all duration-200 cursor-pointer"
              >
                ...
                <div className="h-11 w-11 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center group-hover:bg-white/25 group-hover:text-white shrink-0 shadow-3xs">
            ...
            {/* ===================== RECENT ACTIVE APPOINTMENTS TRACKER TIMELINE ===================== */}
              ...
              <div className="mt-10 border-t border-slate-100 pt-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {userAppointments.slice(0, 2).map((apt) => {
                    ...
                    return (
                      <div key={apt._id} className="bg-slate-50/70 border border-slate-100 p-5 rounded-3xl shadow-xs hover:shadow-sm transition-all space-y-4">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-extrabold text-slate-900 text-sm">{apt.doctorName || 'General Practice Specialist'}</h4>
                        ...
                        {/* Mentor-pleasing Visual Timeline Tracker Gauge */}
                        <div className="pt-2">
      ...
      <section className="py-12 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="bg-white p-12 text-center rounded-2xl border border-slate-100">
            <p className="text-slate-500 font-medium">No doctors found in directories. Please add some doctors to populate the index.</p>
            {user && (
              <Link
                to="/add-doctor"
                className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl shadow-xs"
              >
